<?php
     ...
     switch ($gjatesi) {
         case 1:if(stripos($kerkim,"@")==-1) $persona=exec_query("select * from student where (s_emri like '%{$fjale[0]}%' or strcmp(soundex(s_emri), soundex('{$fjale[0]}')) = 0 or s_mbiemri like '%{$fjale[0]}%' or strcmp(soundex(s_mbiemri), soundex('{$fjale[0]}')) = 0)", $lidhja);
